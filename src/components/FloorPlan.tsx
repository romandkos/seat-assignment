...
const FloorPlan = (props: FloorPlanProps) => {
  const { sceneId, deskAssignments, addDeskAssignment, removedDeskAssignment } =
    props;
  ...
  const onDrop = (archilogicEvent: any, floorPlan: any) => {
    ...
    const { assets } = floorPlan.getResourcesFromPosition(
      floorPlan.getPlanPosition(position)
    );
